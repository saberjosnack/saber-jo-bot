...
router.post("/forgot-password", async (req, res) => {
  ...
  // ما منفصح إذا الإيميل موجود ولا لأ (حماية خصوصية)
  if (employee) {
    employee.resetToken = uuid();
    employee.resetTokenExpires = Date.now() + 60 * 60 * 1000; // ساعة
    store.write("employees.json", employees);

    const resetLink = `${req.protocol}://${req.get("host")}/dashboard?token=${employee.resetToken}`;
    await sendPasswordResetEmail(employee.email, resetLink);
  }

  res.json({ message: "إذا الإيميل مسجل عندنا، رح توصلك رسالة فيها رابط الاسترجاع." });
});
...
router.post("/reset-password", async (req, res) => {
  ...
  if (!employee || employee.resetTokenExpires < Date.now()) {
    return res.status(400).json({ error: "الرابط منتهي أو غير صالح، اطلب رابط جديد." });
  }
  ...
  res.json({ message: "تم تغيير كلمة المرور، فوت سجل دخول." });
});
