...
// بس السوكت بينعمل فوراً لحظة بدء أي محاولة اتصال (حتى قبل ما يخلص أي مصادقة)، فلو حاولنا نستخدمه وهو لسا
// "عم ينتظر مسح QR" أو "قاطع"، العملية إما تفشل بخطأ غامض أو ترجع نتيجة فاضية بصمت (متل ما صار بجلب الجروبات).
function requireConnectedSock(botId) {
  const conn = connections.get(botId);
  if (!conn?.sock || conn.status !== "connected") {
    throw new Error("هاد البوت مش متصل بواتساب فعلياً بعد — امسح رمز QR من صفحة الربط الأول.");
  }
  return conn;
}
